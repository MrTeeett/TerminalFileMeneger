...
func TestRenderFilePreviewBody_TextFallbackAndCache(t *testing.T) {
	...
	if out[0] != "AAA " {
		t.Fatalf("line0 = %q; want 'AAA '", out[0])
	}
	// width=4 trims long line to 3 chars + ellipsis
	if out[1] != "BBB…" {
		t.Fatalf("line1 = %q; want 'BBB…'", out[1])
	}
	if sp.calls != 1 {
	...
	if sp.calls != 1 {
		t.Fatalf("provider called again unexpectedly: %d", sp.calls)
	}
}
