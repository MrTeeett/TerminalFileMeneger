...
func TestRefreshSortAndHidden(t *testing.T) {
	...
	if p.Cwd != dir {
		t.Fatalf("Cwd = %q; want %q", p.Cwd, dir)
	}
	// Expect: directories first (sorted), then files; hidden excluded
	if len(p.Entries) != 3 {
		t.Fatalf("len(entries)=%d; want 3", len(p.Entries))
	}
	if !(p.Entries[0].IsDir && p.Entries[0].Name == "Adir") {
		t.Fatalf("first entry = %#v", p.Entries[0])
	}
	if !(p.Entries[1].IsDir && p.Entries[1].Name == "Bdir") {
		t.Fatalf("second entry = %#v", p.Entries[1])
	}
	if p.Entries[2].IsDir || p.Entries[2].Name != "z.txt" {
		t.Fatalf("third entry = %#v", p.Entries[2])
	}
	if p.MaxDirName != 5 {
		t.Fatalf("MaxDirName=%d; want 5", p.MaxDirName)
	}
}
...
func TestChdirFiltersAndMaxDir(t *testing.T) {
	...
	if err := os.Mkdir(filepath.Join(sub, "subdirlong"), 0o755); err != nil {
		t.Fatal(err)
	}
	...
	if p.Cwd != sub {
		t.Fatalf("Cwd after chdir = %q; want %q", p.Cwd, sub)
	}
